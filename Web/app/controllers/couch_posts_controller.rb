class CouchPostsController < ApplicationController
  ...
  def index
    @couch_posts = CouchPost.joins('LEFT OUTER JOIN users ON couch_posts.user_id = users.id').order('users.premium_expiration IS NULL, users.premium_expiration DESC')
    @showingall = true
